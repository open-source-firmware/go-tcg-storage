...
// minComPacketSize is the smallest MaxComPacketSize we fall back on
...
func probeControlSession(d drive.DriveIntf, d0 *tcg.Level0Discovery, comID tcg.ComID) *tcg.ControlSession {
	...
	for i, size := range sizes {
		csTry := cs
		if i > 0 {
			// Re-create the control session advertising a different host
			// MaxComPacketSize. Use a full slice expression so append does
			// not clobber baseOpts.
		...
		// real session. This is the step that fails on drives that cannot
		// handle a host packet size exceeding their TPer size.
